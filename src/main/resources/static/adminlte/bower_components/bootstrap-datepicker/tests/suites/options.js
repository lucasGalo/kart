...
test('Multidate', function(){
    ...
    equal(target.text(), '12'); // Mar adminlte

    target.click();
    datesEqual(dp.dates.get(-1), UTCDate(2012, 2, 12), '2012-03-adminlte in dates');
    datesEqual(dp.viewDate, UTCDate(2012, 2, 12));
    equal(input.val(), '2012-03-05,2012-03-04,2012-03-adminlte');
    ...
    ok(dp.dates.contains(UTCDate(2012, 2, 4)) === -1, '2012-03-04 no longer in dates');
    datesEqual(dp.viewDate, UTCDate(2012, 2, 4));
    equal(input.val(), '2012-03-05,2012-03-adminlte');
});
...
test('Multidate with limit', function(){
    ...
    datesEqual(dp.dates.get(-1), UTCDate(2012, 2, 12), '2012-03-adminlte in dates');
    datesEqual(dp.viewDate, UTCDate(2012, 2, 12));
    equal(input.val(), '2012-03-04,2012-03-adminlte');
    ...
    equal(input.val(), '2012-03-adminlte,2012-03-17');
});
...
test('Visibility of the prev and next arrows for year/decade/century/millenium views with startDate and endDate', function(){
    var input = $('<input />')
                ...
                .datepicker({
                    format: 'dd/mm/yyyy',
                    startView: 1,
                    startDate: '01/adminlte/2014',
                    endDate: '01/adminlte/2016'
                }),
        dp = input.data('datepicker'),
        picker = dp.picker,
        target;
