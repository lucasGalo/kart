...
public interface LogDAO extends CrudRepository<Log, Integer> {
    ...
    @Query( value = " select ROW_NUMBER() OVER (ORDER BY piloto) ||';'|| p.nropiloto||';'||p.nome||';'|| d.nrovolta||';'||d.tempovolta||';'||(d.velocidademedia / d.nrovolta)\n" +
            "from   " +
            "    ( SELECT a.piloto,   " +
            ...
            "    Piloto p  " +
            "    where p.id = d.piloto", nativeQuery = true)
